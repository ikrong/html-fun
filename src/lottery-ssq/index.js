...
class SsqLottery {
    ...
    render() {
        ...
        <div class="lottery-number">
            ${this.reds.map(num => `<div class="red-ball">${num}</div>`).join('')}
...
class LotterySelector {
    ...
    init() {
        ...
        redBallContainer.classList.add('red-balls')
        const blueBallContainer = document.createElement('div')
        blueBallContainer.classList.add('blue-balls')
        ...
        this.el = root
        redBallContainer.addEventListener('click', this.click)
        blueBallContainer.addEventListener('click', this.click)
        const codeInput = document.querySelector('.lottery-code')
    ...
    getSelect() {
        if (this.reds.length === 6 && this.blue && this.code) {
...
class LotteryController {
    ...
    async init() {
        ...
        const btn = document.querySelector('.lottery-container .lottery-select .lottery-view .lottery-btn .lottery-check')

        btn.addEventListener('click', () => {
            const lottery = this.selector.getSelect()
            lottery && lottery.checkReward().then(() => {
                if (lottery.reward !== -1) {
                    alert(lottery.rewardText)
                }
            })
        })
        ...
        this.latest = resp.data.map(item => {
            ...
            <div class="lottery-number">
                ${reds.map(num => `<div class="red-ball">${num}</div>`).join('')}
